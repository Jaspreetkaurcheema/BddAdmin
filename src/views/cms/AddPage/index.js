            ...
            <AdaptableCard>
                <div className="max-w-[800px] mx-auto">
                    <div className="flex justify-between items-center mb-4">
                        <h3>
                            {mode === 'edit' && <span>Edit {article?.data?.title}</span>}
                            {mode === 'add' && <span>Add Page</span>}
                            {mode === 'preview' && <span>Preview Page</span>}
                        </h3>
                        {mode === 'preview' ? (
                            <Button
                                onClick={() =>
                                    onModeChange(id > 0? 'edit' : 'add')
                                }
                                size="sm"
                            >
                                Back
