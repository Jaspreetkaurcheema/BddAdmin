...
import { apiEditArticle, apiPostArticle } from 'services/KnowledgeBaseService'
...
const validationSchema = Yup.object().shape({
    ...
    slug: Yup.string().required('Category required'),
...
const Editor = ({ mode }) => {
    ...
    console.log(article, 'articlearticle')
    ...
    const onComplete = async (value, setSubmitting) => {
        ...
        if (mode == "edit") {
            resp = await apiEditArticle(newData ,id)
        }
    ...
    return (
        <Formik
            initialValues={{
                ...
                content: article?.data?.content ? article?.data?.content : '',
                slug: article?.data?.slug ? article?.data?.slug : '',
            }}
            enableReinitialize
