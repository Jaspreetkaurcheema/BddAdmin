...
const PoolsMembers = ({ setSelectedCard, selected }) => {
   ...
   const lastRoute = pathname.split('/').pop()
    ...
    const total = useSelector((state) => state.poolsMembers.data.customerList.total_rows_count)
    const data = useSelector((state) => state.poolsMembers.data.customerList.data)
    const loading = useSelector((state) => state.poolsMembers.data.loading)
    // const filterData = useSelector(
    //     (state) => state.crmCustomers.data.filterData
    // )
   
    const { pageNumber, pageSize, poolId,search } = useSelector(
        (state) => state.poolsMembers.data.tableData
    )


    console.log(lastRoute,'typetypetype')
    // {
    //     "isActive": true,
    //     "search": "string",
    //     "pageSize": 0,
    ...
    console.log(pollid , "checking teh ",columns);
    if(type == 'pool-Req'){
        dispatch(getCustomers({ pageNumber:1, pageSize, poolId:pollid,search, joined:  false}))
    }else{
        dispatch(getCustomers({ pageNumber:1, pageSize, poolId:pollid,search, joined:((lastRoute =='poolRequests') ? false: true)}))

    }
    }, [pageNumber, pageSize, poolId,search,   dispatch,lastRoute])
    ...
    useEffect(() => {
        ...
        fetchData()
    }, [fetchData, pageNumber, pageSize, poolId,search,lastRoute])
               ...
               /> : (
                   <div className="h-full flex flex-col items-center justify-center">
                       <DoubleSidedImage
                           ...
                           alt="No user found!"
                       />
                       <h3 className="mt-8">{type == 'pool-Req'? "No Pool Requests Found" :"No Pool Members Found"}</h3>
                   </div>
               )
           }
