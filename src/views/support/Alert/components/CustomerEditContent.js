...
const CustomerEditContent = forwardRef((_, ref) => {
    ...
    const data = useSelector((state) => state.crmCustomers.data.customerList.faq
)

    const { id } = customer

    console.log(id,'datdaddat')
    const onFormSubmit = (values) => {
        ...
        if (!isEmpty(editedCustomer)) {
            ...
            dispatch(getCustomers({pageNumber:1,pageSize:10,search:'',userType:1}))
