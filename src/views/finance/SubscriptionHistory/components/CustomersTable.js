...
const Customers = () => {
    const dispatch = useDispatch()
    const data = useSelector((state) => state.crmSubscriptionHistory.data.customerList.data) || null
    const [isVisible, setIsVisible] = useState(false)
    const loading = useSelector((state) => state.crmSubscriptionHistory.data.loading)
    const filterData = useSelector(
        (state) => state.crmSubscriptionHistory.data.filterData
    )
    const columns = [
        ...
        },
        {
            ...
            cell: (props) => {
                ...
                return (
                    <div className="flex items-center">
                        {dayjs(row.transaction_date).format('MM/DD/YYYY')}
                    </div>
                )
            },
        },
