...
const columns = [
            ...
            return (
                <div className="flex items-center">
                    ...
                    <span className="ml-2 rtl:mr-2 capitalize">
                        {row.subscription_name}/{row.subscription_period}
                    </span>
                </div>
            )
        },
    ...
    {
        header: 'Subscription_Stroage_MB',
        accessorKey: 'subscriptionstroage',
        cell: (props) => {
            const row = props.row.original

            return (
                <div className="flex items-center justify-end"style={{ width: '55%' }}>
                   
                    <span className="ml-2 rtl:mr-2 capitalize">
                        {row.subscription_storage}
                    </span>
                </div>
            )
        },
    ...
    {
        header: 'Subscription_Charges',
        accessorKey: 'subscriptioncharges',
        cell: (props) => {
            const row = props.row.original
...
const Customers = () => {
    ...
    const data = useSelector((state) => state.crmSubscription.data.customerList)|| null
    const loading = useSelector((state) => state.crmSubscription.data.loading)
    const filterData = useSelector(
        (state) => state.crmSubscription.data.filterData
    )

    let { pageIndex, pageSize, sort, query, search, total } = useSelector(
        (state) => state.crmSubscription.data.tableData
    )
