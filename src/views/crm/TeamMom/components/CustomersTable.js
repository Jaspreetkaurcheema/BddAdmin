...
import TeamMomDeleteConfirmation from './TeamMomDeleteConfirmation'
...
const Customers = () => {
    const dispatch = useDispatch()
    const data = useSelector((state) => state.crmTeamMom.data.customerList.data)
    const [isVisible , setIsVisible] = useState(false)
    const loading = useSelector((state) => state.crmTeamMom.data.loading)
    const filterData = useSelector(
        (state) => state.crmTeamMom.data.filterData
    )
...
const columns = [
        ...
        cell: (props) => {
            ...
            return (
                <div className="center">
                   ...
                   <SliderOpen name={"Update Plan"} subscription_id={row.subscription_id} row={row} dispatch ={()=>dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:9})) } userType={9}  />
    ...
    const fetchData = useCallback(() => {
        dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:9}))
    ...
    return (
        <>
         <TeamMomDeleteConfirmation isVisible={isVisible} setIsVisible={setIsVisible}    />
         <>{data == null ? <>
                    ...
                    /> : (
                        <div className="h-full flex flex-col items-center justify-center">
                            <DoubleSidedImage
                                ...
                                alt="No user found!"
                            />
                            <h3 className="mt-8">No Team Mom found!</h3>
                        </div>
                    )
                }</>} </>
