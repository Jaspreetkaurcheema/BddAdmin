...
import FamilyDeleteConfirmation from './FamilyDeleteConfirmation'
...
const Customers = () => {
    const dispatch = useDispatch()
    const data = useSelector((state) => state.crmFamily.data.customerList.data)
    const [isVisible , setIsVisible] = useState(false)
    const loading = useSelector((state) => state.crmFamily.data.loading)
    const filterData = useSelector(
        (state) => state.crmFamily.data.filterData
    )
    let { pageIndex, pageSize, sort, query, search, total } = useSelector(
        (state) => state.crmFamily.data.tableData
    )
const columns = [
    {
        header: 'Username',
        ...
        cell: (props) => {
            ...
            return (
                <div className="center">
                   ...
                   <SliderOpen name={"Update Plan"} subscription_id={row.subscription_id} row={row} dispatch ={()=>dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:6})) } userType={6}  />
                </div>
            )
        },
    },
    ...
    const fetchData = useCallback(() => {
        dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:6}))
    ...
    return (
        <>
        <FamilyDeleteConfirmation isVisible={isVisible} setIsVisible={setIsVisible}    />
        <>{data == null ? <>
                    ...
                    /> : (
                        <div className="h-full flex flex-col items-center justify-center">
                            <DoubleSidedImage
                                ...
                                alt="No user found!"
                            />
                            <h3 className="mt-8">No Family found!</h3>
                        </div>
                    )
                }</>} </>
