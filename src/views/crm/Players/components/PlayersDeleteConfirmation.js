...
const PlayersDeleteConfirmation = ({isVisible,setIsVisible}) => {
    const dispatch = useDispatch()
    const dialogOpen = useSelector(
        (state) => state.crmPlayers.state.deleteConfirmation
    )
    const selectedProduct = useSelector(
        (state) =>  state.crmPlayers.state.selectedCustomer
    )
    ...
    const onDelete = async () => {
        ...
        if (success) {
            dispatch(getCustomers({id:4,...tableData}))
            setIsVisible(false)
            toast.push(
                <Notification
    ...
    return (
        ...
        >
            <p>
                Are you sure you want to delete this Players ? This action cannot be
                undone.
            </p>
        </ConfirmDialog>
    )
...
export default PlayersDeleteConfirmation
