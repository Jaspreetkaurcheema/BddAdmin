...
import RefereeDeleteConfirmation from './RefreeDeleteConfirmation'
...
const Customers = () => {
    const dispatch = useDispatch()
    const data = useSelector((state) => state.crmReferee.data.customerList.data)
    const [isVisible , setIsVisible] = useState(false)
    const loading = useSelector((state) => state.crmReferee.data.loading)
    const filterData = useSelector(
        (state) => state.crmReferee.data.filterData
    )
        ...
        cell: (props) => {
            ...
            return (
                <div className="center">
                   ...
                   <SliderOpen name={"Update Plan"} subscription_id={row.subscription_id} row={row} dispatch ={()=>dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:5})) } userType={5}  />
...
gamesParticipated}
    ...
    let { pageIndex, pageSize, sort, query, search, total } = useSelector(
        (state) => state.crmReferee.data.tableData
    )
    ...
    const fetchData = useCallback(() => {
        dispatch(getCustomers({ pageIndex, pageSize, sort, query, search, filterData ,id:5}))
    ...
    return (
        <>
        <RefereeDeleteConfirmation isVisible={isVisible} setIsVisible={setIsVisible}    />
        <>{data == null ? <>
                    ...
                    /> : (
                        <div className="h-full flex flex-col items-center justify-center">
                            <DoubleSidedImage
                                ...
                                alt="No user found!"
                            />
                            <h3 className="mt-8">No Referee found!</h3>
                        </div>
                    )
                }</>} </>
