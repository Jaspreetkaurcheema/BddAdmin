...
const RefereeDeleteConfirmation = ({isVisible,setIsVisible}) => {
    const dispatch = useDispatch()
    const dialogOpen = useSelector(
        (state) => state.crmReferee.state.deleteConfirmation
    )
    const selectedProduct = useSelector(
        (state) => state.crmReferee.state.selectedCustomer
    )
    ...
    const onDelete = async () => {
        ...
        if (success) {
            dispatch(getCustomers({id:5,...tableData}))
            setIsVisible(false)
            toast.push(
                <Notification
...
export default RefereeDeleteConfirmation
