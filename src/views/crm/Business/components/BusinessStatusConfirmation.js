...
const BusinessStatusConfirmation = ({status_id, isVisible,setIsVisible}) => {

    const [remarks,setRemark]=useState('')
   
    const dispatch = useDispatch()
    const dialogOpen = useSelector(
        (state) => state.crmBusiness?.state?.statusConfirmation
    )
    const selectedProduct = useSelector(
        (state) =>  state.crmBusiness?.state?.selectedCustomer
    )
    const tableData = useSelector(
        (state) => state.crmBusiness?.data?.tableData
    )
    ...
    const onStatus = async () => {
        ...
        if (success) {
            dispatch(getCustomers({id:7,...tableData}))
            setIsVisible(false)
            toast.push(
                <Notification
                    title={'Successfuly'}
...
export default BusinessStatusConfirmation
