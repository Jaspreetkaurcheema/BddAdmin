...
const Customers = () => {
    const dispatch = useDispatch()
    const data = useSelector((state) => state.crmGames.data.customerList) || null
    const [isVisible , setIsVisible] = useState(false)
    const loading = useSelector((state) => state.crmGames.data.loading)
    const filterData = useSelector(
        (state) => state.crmGames.data.filterData
    )
    const columns = [
       
        {
            header: 'Sports',
            accessorKey: 'gameperteam',
            cell: (props) => {
                    ...
                    /> : (
                        <div className="h-full flex flex-col items-center justify-center">
                            <DoubleSidedImage
                                ...
                                alt="No user found!"
                            />
                            <h3 className="mt-8">No Games found!</h3>
                        </div>
                    )
                }</>} </>
